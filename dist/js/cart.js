...
function _createCheckoutSession() {
  _createCheckoutSession = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2(lineItems) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (payBtn) {
    payBtn.addEventListener("click", function () {
      var lineItems = getLineItemsFromCart(); // get items from local storage

      // Validate the data
      if (!Array.isArray(lineItems) || lineItems.some(function (item) {
        return !item.price_data || typeof item.price_data.unit_amount !== 'number';
      })) {
        console.error('Invalid cartItems data');
        return;
      }
      createCheckoutSession(lineItems).then(function (session) {
        if (session && session.id) {
  ...
  if (checkoutButton) {
    checkoutButton.addEventListener('click', /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
      var lineItems, sessionData;
